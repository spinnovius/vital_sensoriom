...
<table width="100%;">
	<tr>
		<td><b style="font-size: 16px;">{{$fname}}</b></td>
		<td><b style="font-size: 16px;">{{@$age}} Y/{{$gender}}</b></td>
...
->where('patient_general_examination.patient_id','=',$patientid)
->groupBy("patient_general_examination.id")
->get();
$systemexamination=DB::table('system examination')->where('patient_id',$patientid)->get();
@endphp
@if ($examination)
<div class="row">
  <div class="col-md-12">
...
->where('appointment_details.patient_id',$patientid)
->where('doctodoc.id','!=','')
->groupBy('doctors.id')
->orderby('idname','ASC')
->get();
