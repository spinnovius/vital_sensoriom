                        ...
                        <tbody>  
                          @foreach($PatientDetail as $vitals)
                                <tr>
                                    ...
                                    @if(session('role')==2)
                                    <td id='arrive_{{ $vitals->id }}'><label style="cursor: pointer;" id='{{ $vitals->id }}' class="label @if(session('role')==5) btnclk @endif {{ ($vitals->arrives == 0) ? 'label-info' : 'label-success' }}">{{ ($vitals->arrives == 0) ? 'Arrive' : 'coming' }}</label></td>
                                    @endif
                                    @if(session('role')==5)
                                    <td>
                                        @if($vitals->flag==0)
