...
class AdminController extends Controller {
    ...
    public function register(Request $request)
    {
        try {
                ...
                $check_email = User::where('contact_number',$request->contact_number)->count();
                
                if($check_email){
                    $response = array(
                            "data" => (object)[],
                            "error_code" => 1,
                            "message" => 'Contactnumber is already exist' 
                        );
                }
...
$strtotime = strtotime($request->dob);
$age = date("Y-m-d", $strtotime);    
//dd($request->age);
if($age){
$agetimestamp = strtotime($age);
$age_date = date("Y-m-d", $agetimestamp);
$diff = date_diff(date_create($age), date_create(date("Y-m-d")));
$pyear = $diff->format('%y');
}else{
$pyear = 18;
}
                    ...
                    elseif($request->role_id == 2)
                    {
                        ...
                        $dd=DoctorDetail::where('mbbs_registration_number',request('mbbs_registration_number'))->first();
                        
                        $dd1=DoctorDetail::where('md_ms_dnb_registration_number',request('md_ms_dnb_registration_number'))->first();
                        
                        $dd2=DoctorDetail::where('dm_mch_dnb_registration_number',request('dm_mch_dnb_registration_number'))->first();
                        if ($dd && $dd1|| $dd || $dd1 || $dd2) {
                            $response = array(
                                        "data" => (object)[],
                                        "error_code" => 1,
                                        "message" => 'dr_register_no already exists' 
                                    );
                            return response()->json($response);
                            exit;
                        }
                        else
                        {
                           ...
                           // $docror->register_no=request('dr_register_no');
                            $doctor->save();
                    ...
                    Mail::send('emailTemplate.registration', ['parameter' => $para], function ($m) use($data) {
                                ...
                                $m->to($data['to_email'])->subject('Sensoriom | New registration for Sensoriom');
                        });
    ...
    public function userprofile(Request $request)
    {
        ...
        try {
                ...
                if($role == 4)
                {
                    ...
                    $dateOfBirth = date("d-m-Y", strtotime($patient_detail->dob));
                    $today = date("Y-m-d");
                ...
                elseif($role == 2)
                {
                    ...
                    ->where('users.id', $request->user_id)
                    ->orderBy('doctors_balance.id', 'DESC')
                    ->first();
                   ...
                   if(@$doctor_detail->mbbs_registration_number == null)
                    ...
                    //dd($user->id);
			        $amount=DB::table('doctors_balance')->where('doctor_id',$user->id)->  orderby('id','DESC')->get();
			        $DoctorBalance =DoctorBalance::where('doctor_id',$user->id)->latest()->first();
                    ...
                    //dd(array_sum($online_amount));
                    if($amount){
                    $online_amount=array_sum($online_amount);
                    $debitAmount=array_sum($debitAmount);
                    $diff=$online_amount-$debitAmount; 
                    }else{
                    $diff=0;    
                    }
                    //dd($diff);
                    $doctor = array(
                        ...
                        'document' => $document,
                        'available' => $available,
                        'doctor_balance' => isset($DoctorBalance)?$DoctorBalance->total_amount:"",//$total_amount,
        ...
        try {
                ...
                if($role == 4)
                {
                    ...
                    $response = array(
                        ...
                        $patient_detail->clinic_id = 1;
                        $patient_detail->patient_id = $user->id;
                        $patient_detail->save();
                        $patient = PatientDetail::whereId($patient_detail->id)->update([
                        'profile_pic' => $profile_pic1,
                        ]);
                ...
                elseif($role == 3)
                {
                    ...
                    $response = array(
                        ...
                        "data" => '',
                        "error_code" => 0,
                        "message" => 'There no Data Coach Detail' 
                        );
                    }
                }
                elseif($role == 2)
                {
                    ...
                    $response = array(
                        ...
                        "data" => '',
                        "error_code" => 0,
                        "message" => 'There no Data Doctor Detail' 
                        );
                    }
                }
        } 
        ...
        // try {
                ...
                if($usercount)
                {
                    ...
                    if($user_check_verified)
                    {
                        ...
                        $mail = Mail::send('emailTemplate.forgotpassword', ['parameter' => $para], function ($m) use($data) {
                                    $m->from('hello@sensoriom.com', 'Sensoriom');
                                    $m->to($data['to_email'])->subject('Sensoriom | Forgot password for Sensoriom');
                            });
