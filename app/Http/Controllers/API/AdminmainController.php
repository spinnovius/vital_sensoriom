...
class AdminmainController extends Controller
{
    ...
    public function dashboard(Request $request) {
        ...
        if (Auth::check())
        {
            ...
            if($id->role_id==2)
            {
                ...
                // $patient_all_detail = Appointments::select('d.fname as doctorname','p.fname as patientname','appointment_details.*')
                //     //->join('doctors','appointment_details.doctor_id','=','doctors.id')
                //     ->join('users as d','appointment_details.doctor_id','=','d.id')
                //     ->join('users as p','appointment_details.patient_id','=','p.id')
                //     ->where('appointment_details.doctor_id','=',$user_id)
    ...
    public function allpatient(Request $request){
        ...
        if($id->role_id==5)
        {
            // $patient_detail=Appointments::select('patient_detail.dob','patient_detail.patient_id','patient_detail.dob','patient_detail.age as age','patient_detail.gender', 'users.contact_number','users.email','d.fname as Dfname','appointment_details.*','users.*')
            // ->join('users', 'appointment_details.patient_id', '=', 'users.id')
            ...
            // ->where('appointment_details.clinic_id','=',$id->id)
            // //->orderby('uniqueid','desc')
            // //->distinct()
            // ->get();
            //dd($patient_detail);
    ...
    public function indexLabtests($patient_id){
        $lab_detail=PatientLabDetail::select('patient_lab_detail.id as uniqueid','patient_lab_detail.*')
                ...
                ->orderBy('patient_lab_detail.test_name')
                ->get();
...
//            dd($new_date);
    ...
    public function indexProcedure($patient_id){
        $patient_detail=PatientProcedure::select('patient_procedure.id as uniqueid','patient_procedure.*')
                ...
                ->orderBy('patient_procedure.procedure_name','ASC')
                ->get();
            ...
            if($days)
            {
                ...
                $id=Auth::user()->id;
                    foreach ($days as $key => $e) {
                        ...
                        //dd($request->$starttime);
                        foreach ($request->$starttime as $key => $value) {
                            $DoctorDays = DoctorDays::where('user_id','=',$id)->where('days','=',$e)->count();
                            //dd($DoctorDays);
                            if($DoctorDays){
                                $DoctorDays = DoctorDays::where('user_id','=',$id);
                            }else{
                                $DoctorDays = new DoctorDays();
                            }
                            //dd($DoctorDays);
                            $DoctorDays->user_id = $id;
                            $DoctorDays->days = $e? $e :"";
                            $DoctorDays->start_time = $request->$starttime[$key];
                            $DoctorDays->end_time = $request->$endtime[$key];
                            $DoctorDays->time_slot = $request->timeslot;
    ...
    public function indexClinicalManagement($patient_id)
    {
        ...
        $lab_detail=PatientPriscription::select('patient_priscription.id as uniqueid','patient_priscription.*','d.fname as doctorname')
                ...
                ->where('patient_priscription.patient_id','=',$patient_id)
                ->get();
        return view('admin_main.healthdata.healthdata_CM',["patient_id" => $patient_id,'patient_detail'=>$patient_detail,'city'=>$city,'speciality'=>$speciality,'hospital'=>$hospital,'lab_detail'=>$lab_detail,'refer'=>$refer]);
    }
    public function StoreClinicalManagement(Request $request,$patient_id)
    {
        ...
        $pp->doctor_id=$dd->doctor_id;
        $pp->diagnosis = isset($request->diagnosis)?$request->diagnosis:'';
        $pp->medicine_name = request('drugname');
        $pp->dose = request('dose');
        $pp->freq = request('frequency');
        $pp->route = request('route');
        $pp->duration = request('duration');
        $pp->notes = request('notes');
        $pp->save();
        //Advice Treatment
        $goal = request('goal');
        $no = request('no');
        ...
        $Appointments=new Appointments();
        $Appointments->user_id = request('reasonforreferral');//user_id
        $Appointments->clinic_id = $dd->clinic_id;
        $Appointments->patient_id = $patient_id;
        $Appointments->city_id = request('city');
        $Appointments->hostipal_id = request('hostipal');
        $Appointments->speciality_id = request('speciality');
        $Appointments->doctor_id = $dd->doctor_id;
        $Appointments->date = date('Y-m-d', strtotime("+1 day"));
        $Appointments->time = '1:15 PM';
        $Appointments->flag = 0;
        $Appointments->role = 6;
        $Appointments->reason_refer = request('reasonforreferral');
        $Appointments->save();
        //
        $AdviceTreatment = new AdviceTreatment();
        $AdviceTreatment->doctor_id=$dd->doctor_id;
        $AdviceTreatment->patient_id=$patient_id;
        $AdviceTreatment->date=request('date');
        $AdviceTreatment->investigation_name = request('investigationnotes');
        $AdviceTreatment->city = request('city');
    ...
    public function referral(Request $request)
    {
        ...
        $id = Auth::user()->id;
            ...
            select('doctorrefer.*','pd.age as age','pd.gender as gender','patient.fname as patientname','patient.id as patient_id','doc.fname as Dfname','city.city as city','C.fname as Cfname')//,'doctor_speciality_select.speciality_id as idname'
            ->leftjoin('patient_detail as pd','doctorrefer.user_id','=','pd.patient_id')
            ->leftjoin('users as patient','doctorrefer.user_id','=','patient.id')
            ->leftjoin('users as doc','doctorrefer.doc_id','=','doc.id')
            ->leftjoin('doctors','doctorrefer.doc_id','=','doctors.doctor_id')
          ...
          //old bhargav 10-02-2020
           /*$patient_detail=Appointments::select('appointment_details.*','p.fname as patientname','pd.age as age','pd.gender as gender','d.fname as Dfname','doctor_speciality.speciality as speciality','C.fname as Cfname','city.city as city')
           ->leftjoin('users as p','appointment_details.patient_id','=','p.id')
           ->leftjoin('patient_detail as pd','p.id','=','pd.patient_id')
           //doctor
           ->leftjoin('users as d','appointment_details.doctor_id','=','d.id')
           ->leftjoin('doctors','d.id','=','doctors.doctor_id')
           ->leftjoin('doctor_speciality','doctors.speciality','=','doctor_speciality.id')
           ->leftjoin('city','doctors.city','=','city.id')
           //
           ->leftjoin('users as C','doctors.clinic_id','=','C.id')
